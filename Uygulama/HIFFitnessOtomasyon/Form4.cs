...
namespace HIFFitnessOtomasyon
{
    public partial class Form4 : Form
    {
        ...
        {
            bag.Open();
            string dondurkayit = "insert into dbo.DondurulanUye(AD,SOYAD,TC_NO,TEL_NO,E_MAIL,D_TARIHI,KAN_GRUBU,KAYIT_TARIHI,GUNCEL_TARIH) values (@ad,@soyad,@tc,@tel,@mail,@dtarih,@kan,@kayit,@guncel)";
        ...
        {
            DialogResult dialogResult = MessageBox.Show("Seçilen üyenin kaydı dondurulacaktır!. Devam Edilsin mi?", "UYARI", MessageBoxButtons.YesNo);
            if (dialogResult == DialogResult.Yes)
            {
                if (this.dataGridView1.SelectedRows.Count > 0)
                {
                    Kayit();
                    bag.Open();
                    string sil = "DELETE from UYELER where ID='" + dataGridView1.CurrentRow.Cells[0].Value.ToString() + "'";
                    SqlCommand komut = new SqlCommand(sil, bag);
                    komut.ExecuteNonQuery();
                    bag.Close();
                    MessageBox.Show("Dondurma işlemi Başarılı!");
                }
                else
                {
